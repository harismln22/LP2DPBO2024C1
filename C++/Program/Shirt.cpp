...
class Shirt : public Clothing
{
	...
	public:
		...
		{
			if (!listShirt.empty()) // jika list tidak kosong
			{
				// inisialisasi dengan mengambil setiap nama label tabel untuk perbandingan string paling panjang nanti
				...
				// memasukkan semuanya ke dalam array
				...
				cout << "|| "; // pagar tabel
				for (int i = 0; i < sizeof(headers)/sizeof(headers[0]); i++) // loop dan print semua header (Nama label tabel)
				{
					cout << headers[i]; // print
					for (int j = 0; j < maxLengths[i] - headers[i].length() + 1; j++) 
					{
						cout << " "; // spasi untuk menyamakan semua lebar tabel
					}
					cout << "|| "; // pagar tabel
				}
				cout << endl;

				for (Shirt shirt : listShirt) // loop semua list dari 'listShirt'
				{
					// menyimpan setiap input ke array 'data'
					...
					cout << "|| "; // pagar tabel
					for (int i = 0; i < sizeof(data)/sizeof(data[0]); i++) // loop dan print semua input
					{
						cout << data[i]; // print input
						for (int j = 0; j < maxLengths[i] - data[i].length() + 1; j++) 
						{
							cout << " "; // spasi untuk menyamakan semmua lebar tabel
						}
						cout << "|| "; // pagar tabel
					}
					cout << endl; 
